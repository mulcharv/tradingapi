...
app.get('/stocks/:stockid/interval/:intervalid', passport.authenticate('jwt',  {session: false}), asyncHandler(async(req, res, next) => {
  if (Number(req.params.intervalid) === 1) {
  ...
  if (Number(req.params.intervalid) === 7) {
    url = `https://api.marketstack.com/v1/intraday?access_key=${marketstack}&symbols=${req.params.stockid}&date_from=${dateFromfnl}&date_to=${dateTofnl}&interval=10min&limit=500`;
  }
    ...
    url = `https://api.marketstack.com/v1/intraday?access_key=${marketstack}&symbols=${req.params.stockid}&date_from=${dateFromfnl}&date_to=${dateTofnl}&interval=6hour&limit=370`;
  }
  const response = await fetch(url);
  const data = await response.json();
