...
app.get('/stocks/:stockid/interval/:intervalid', passport.authenticate('jwt',  {session: false}), asyncHandler(async(req, res, next) => {
  ...
  if (Number(req.params.intervalid) === 7) {
    url = `https://api.marketstack.com/v1/intraday?access_key=${marketstack}&symbols=${req.params.stockid}&date_from=${dateFromfnl}&date_to=${dateTofnl}&interval=5min&limit=500`;
  }
